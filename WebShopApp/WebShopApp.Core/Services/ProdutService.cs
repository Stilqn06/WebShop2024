...
namespace WebShopApp.Core.Services
{
    public class ProdutService:IProductService
    {
        ...
        public ProdutService(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public bool Update(int ptroductId, string name, int brandId,int categoryId,string picture, int quantity, decimal price, decimal discount)
        {
            var product=GetProductById(ptroductId);
            if (product == default(Product))
            {
                return false;
            }
